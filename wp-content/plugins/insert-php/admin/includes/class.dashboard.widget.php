...
class WINP_Dashboard_Widget extends WINP_Request {
	...
	public function register_hooks() {
		...
		add_filter( 'wbcr/inp/dashboard/widget/print', array( $this, 'dashboard_widget_news' ) );
	}
	...
	public function add_dashboard_widgets() {
		wp_add_dashboard_widget(
			'winp-dashboard-widget',
			__( 'Woody News', 'insert-php' ),
			array( $this, 'dashboard_widget_news' )
		);
	}
	...
	public function get_data() {
		$cached_data = WINP_Plugin::app()->getOption( 'winp_dashboard_widget_news', array() );
		...
		if ( ! empty( $cached_data ) && time() - $cached_data['time'] <= self::WINP_WIDGET_EXPIRATION ) {
			...
			$posts = $this->map_objects( $json, 'Post' );
			if ( $posts ) {
			...
			WINP_Plugin::app()->updateOption(
				'winp_dashboard_widget_news',
				array(
					'banner' => $banner,
					'news'   => $news,
					'time'   => time(),
