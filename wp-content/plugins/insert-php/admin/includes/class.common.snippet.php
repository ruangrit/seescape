...
class WINP_Common_Snippet {
	...
	public function registerHooks()
	{
		...
		add_action( 'edit_form_before_permalink', array( $this, 'editFormBeforePermalink' ) );
		add_action( 'admin_notices', array( $this, 'createUploadsDirectory' ) );
		add_action( 'before_delete_post', array( $this, 'beforeDeletePost' ) );
		add_action( 'save_post', array( $this, 'savePost' ) );
		add_action( 'save_post_' . WINP_SNIPPETS_POST_TYPE, array( $this, 'save_snippet' ), 10, 3 );
		add_action( 'auto-draft_to_publish', array( $this, 'publish_snippet' ) );

		add_filter( 'script_loader_src', array( $this, 'unload_scripts' ), 10, 2 );
	}
	...
	public function createUploadsDirectory()
    {
		...
		if ( ! file_exists( $dir ) ) { ?>
            <div class="notice notice-error is-dismissible">
                ...
                <p><?php _e( 'Please run the following commands in order to make the directory', 'insert-php' ); ?>:
                    <br/><strong>mkdir <?php echo $dir; ?>; </strong><br/><strong>chmod 777 <?php echo $dir; ?>;</strong></p>
            </div>
			<?php return;
		}
		...
		if ( ! wp_is_writable( $dir ) ) { ?>
            <div class="notice notice-error is-dismissible">
                ...
                <p><?php _e( 'Please run the following command to make the directory writable', 'insert-php' ); ?>:<br/><strong>chmod 777 <?php echo $dir; ?> </strong></p>
            </div>
			<?php return;
		}
	...
	public function currentScreen( $current_screen )
	{
		...
		if ( $current_screen->base == 'post' ) {
			add_action( 'admin_head', array( $this, 'currentScreenPost' ) );
		}

		if ( $current_screen->base == 'edit' ) {
			add_action( 'admin_head', array( $this, 'currentScreenEdit' ) );
		}
	}
	...
	public function savePost( $post_id )
    {
		...
		if ( 'external' == $linking ) {
			$before = '/******* Do not edit this file *******' . PHP_EOL .
			          $plugin_title . ' CSS and JS' . PHP_EOL .
		...
		if ( wp_is_writable( WINP_UPLOAD_DIR ) ) {
			$file_content = $before . stripslashes( strip_tags( $content ) ) . $after;
	...
	public function save_snippet( $post_ID, $current_post, $update ) {
		...
		if (
			...
			&& ! empty( $snippet_id )
			&& WINP_SNIPPETS_POST_TYPE == $current_post->post_type
		) {
			$snippet        = array();
	...
	public function unload_scripts( $src, $handle ) {
		...
		if (
			is_admin()
			&& ! empty( $post )
			&& $post->post_type == WINP_SNIPPETS_POST_TYPE
		) {
