...
class WINP_Api extends WINP_Request {
	 ...
	 * @return bool|mixed
	 */
	public function get_all_snippets( $common = false, $parameters = array() ) {
		...
		return $this->map_objects( $json, 'Snippet' );
	}
	...
	public function get_snippet( $id, $common = false ) {
		...
		return $this->map_object( $json, 'Snippet' );
	}
	...
	public function get_all_types() {
		...
		return $this->map_objects( $json, 'Type' );
	}
	...
	public function get_type( $id ) {
		...
		return $this->map_object( $json, 'Type' );
	}
	...
	public function is_changed( $post_id ) {
		...
		if ( ! empty( $data ) && isset( $data['content'] ) ) {
			...
			return $data['content'] != $post->post_content
				|| WINP_Helper::getMetaOption( $post->ID, 'snippet_description' ) != $data['description'];
		} else {
			return true;
		}
	}
