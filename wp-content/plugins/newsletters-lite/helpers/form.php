...
class wpmlFormHelper extends wpMailPlugin {
	...
	function hidden($name = null, $options = array()) {
		...
		?><input type="hidden" name="<?php echo $name; ?>" value="<?php echo esc_attr(wp_unslash($Html -> field_value($name))); ?>" /><?php
	...
	function text($name = null, $options = array()) {
		...
		?><input placeholder="<?php echo esc_attr(wp_unslash($placeholder)); ?>" autocomplete="<?php echo $autocomplete; ?>" <?php echo (empty($tabindex)) ? '' : 'tabindex="' . esc_attr(wp_unslash($tabindex)) . '"'; ?> class="<?php echo $class; ?>" style="width:<?php echo $width; ?>;" id="<?php echo $id; ?>" type="text" name="<?php echo $name; ?>" value="<?php echo esc_attr(wp_unslash($Html -> field_value($name))); ?>" /><?php
	...
	function textarea($name = null, $options = array()) {
		...
		?><textarea style="width:<?php echo $width; ?>;" class="<?php echo $class; ?>" name="<?php echo $name; ?>" id="<?php echo $Html -> field_id($name); ?>" cols="<?php echo $cols; ?>" rows="<?php echo $rows; ?>"><?php echo wp_unslash($Html -> field_value($name)); ?></textarea><?php
	...
	function radio($name = null, $buttons = array(), $options = array()) {
		...
		<?php if (!empty($buttons)) : ?>
			<?php foreach ($buttons as $bkey => $bval) : ?>
				<label><input id="<?php echo $Html -> field_id($name); ?><?php echo $bval; ?>" <?php echo ((!empty($value) && $value == $bkey) || (empty($value) && !empty($default) && $bkey == $default)) ? 'checked="checked"' : ''; ?> onclick="<?php echo $onclick; ?>" type="radio" name="<?php echo $name; ?>" value="<?php echo esc_attr(wp_unslash($bkey)); ?>" /> <?php echo __($bval); ?></label><?php echo $separator; ?>
			<?php endforeach; ?>
		<?php endif; ?>
	...
	function checkbox($name = null, $boxes = array(), $options = array()) {
		...
		<?php if (!empty($boxes)) : ?>
			<?php foreach ($boxes as $bkey => $bval) : ?>
				<label><input <?php echo (is_array($Html -> field_value($name)) && in_array($bkey, $Html -> field_value($name))) ? 'checked="checked"' : ''; ?> type="checkbox" name="<?php echo $name; ?>" id="<?php echo $Html -> field_id($name); ?>checklist<?php echo $bkey; ?>" value="<?php echo esc_attr(wp_unslash($bkey)); ?>" /> <?php echo __($bval); ?></label><?php echo $separator; ?>
			<?php endforeach; ?>
		<?php endif; ?>
