<div id="spamscore_result">
	<?php if (!empty($_POST['spamscore'])) : ?>
		<iframe width="100%" style="width:100%;" frameborder="0" scrolling="no" class="autoHeight widefat" src="<?php echo admin_url('admin-ajax.php?action=newsletters_gauge&value=' . esc_html($_POST['spamscore']) . '&security=' . wp_create_nonce('gauge')); ?>"></iframe>
	<?php else : ?>
...
function spamscorerunner() {
	...
	spamscorerequest = jQuery.ajax({
		data: formvalues,
		dataType: 'xml',
		url: newsletters_ajaxurl + 'action=newsletters_spamscorerunner&security=<?php echo wp_create_nonce('spamscorerunner'); ?>',
		type: "POST",
