...
$user_id = (empty($_POST['user_id'])) ? get_current_user_id() : (int) sanitize_text_field($_POST['user_id']);
...
jQuery(document).ready(function() {
    jQuery('#authorsautocomplete').select2({
	  ...
	  ajax: {
	        url: newsletters_ajaxurl + "action=newsletters_autocomplete_users&security=<?php echo wp_create_nonce('autocomplete_users'); ?>",
	        dataType: 'json',
