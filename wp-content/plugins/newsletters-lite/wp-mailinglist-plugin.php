...
if (!class_exists('wpMailPlugin')) {
	class wpMailPlugin extends wpMailCheckinit {
			...
			if (apply_filters('newsletters_updates', true)) {				
				if ($update = $this -> vendor('update')) {						
			        ...
			        if(empty($version_info['is_valid_key']) || version_compare($this -> version, $version_info["version"], '>=')){
			            ...
			            $option -> response[$plugin_path] -> slug = 'newsletters-lite';
	    ...
	    function ajax_importmultiple() {
			...
			if (!empty($_REQUEST['subscribers'])) {
				...
				foreach ($_REQUEST['subscribers'] as $skey => $subscriber) {					
					...
					$import_preventbu = sanitize_text_field($_REQUEST['import_preventbu']);
					$import_overwrite = sanitize_text_field($_REQUEST['import_overwrite']);
	    ...
	    function ajax_admin_mode() {
		    ...
		    $mode = (empty($_POST['mode'])) ? 'standard' : sanitize_text_field($_POST['mode']);
	    ...
	    function ajax_mailinglist_save() {		    
		    ...
		    $fielddiv = esc_html($_GET['fielddiv']);
		    $fieldname = esc_html($_GET['fieldname']);
		    ...
		    if (!empty($_POST)) {
			    ...
			    if ($mailinglists = $Mailinglist -> select(true)) {
				    foreach ($mailinglists as $id => $title) {
						...
						$checklist .= '<label><input ' . ($id == $insertid || (!empty($_POST['Subscriber']['mailinglists']) && in_array($id, $_POST['Subscriber']['mailinglists'])) ? 'checked="checked"' : '') . ' type="checkbox" name="' . $fieldname . '[]" value="' . esc_attr($id) . '" id="checklist' . $id . '" /> ' . __($title) . ' (' . $Db -> count(array('list_id' => $id)) . ' ' . __('subscribers', 'wp-mailinglist') . ')</label><br/>';
				    }
			    }
	    ...
	    function ajax_posts_by_category() {
			...
			if (!empty($_REQUEST['cat_id']) && $_REQUEST['cat_id'] > 0) {
				$arguments['category'] = map_deep($_REQUEST['cat_id'], 'sanitize_text_field');
			}
			    ...
			    if ($posts = get_posts($arguments)) {
				    ...
				    <ul class="insertfieldslist">
				    	...
				    	<?php foreach ($posts as $post) : ?>
				    		<li>
				    			<span class="insertfieldslistcheckbox"><input type="checkbox" name="insertposts[]" value="<?php echo esc_attr($post -> ID); ?>" id="insertposts_<?php echo $post -> ID; ?>" /></span>

				    			<?php if ($this -> language_do()) : ?>
				    				<span class="insertfieldslistbutton"><a href="" onclick="insert_post('<?php echo $post -> ID; ?>', false); return false;" class="button button-secondary press" title="<?php echo esc_attr(wp_unslash($this -> language_use($_POST['language'], $post -> post_title, false))); ?>"><?php echo $this -> language_use($_POST['language'], $post -> post_title, false); ?></a></span>
				    			<?php else : ?>
				    				<span class="insertfieldslistbutton"><a href="" onclick="insert_post('<?php echo $post -> ID; ?>', false); return false;" class="button button-secondary press" title="<?php echo esc_attr(wp_unslash(__($post -> post_title))); ?>"><?php echo __($post -> post_title); ?></a></span>
				    			<?php endif; ?>
				    		</li>
				    	<?php endforeach; ?>
				    </ul>
				    ...
				    <span>
				    	<button onclick="insert_single_multiple();" class="button button-primary" type="button" name="insert" value="1">
				    		<i class="fa fa-check fa-fw"></i> <?php _e('Insert Selected', 'wp-mailinglist'); ?>
				    	</button>
				    </span>
	    ...
	    function ajax_forms_addfield() {
		    ...
		    $field = $Db -> find(array('id' => sanitize_text_field($_POST['id'])));
	    ...
	    function ajax_forms_deletefield() {
		    ...
		    if (!empty($_POST['field_id']) && !empty($_POST['form_id'])) {
		   		$this -> FieldsForm() -> delete_all(array('field_id' => sanitize_text_field($_POST['field_id']), 'form_id' => sanitize_text_field($_POST['form_id'])));
		   	}
		...
		function ajax_change_themefolder() {
			...
			if (!empty($_POST['themefolder'])) {
				$this -> update_option('theme_folder', sanitize_text_field($_POST['themefolder']));
		...
		function ajax_exportmultiple() {
			...
			global $Html;
			$exportfilename = sanitize_text_field($_REQUEST['exportfile']);
			...
			if ($fp = fopen($exportfilefull, "a")) {
				...
				$delimiter = (empty($_REQUEST['delimiter'])) ? $csvdelimiter : sanitize_text_field($_REQUEST['delimiter']);
		...
		function ajax_subscribe() {
			...
			$widget_id = esc_html($_GET['widget_id']);
			$number = esc_html($_GET['number']);
			$instance = $this -> widget_instance($number);

			$newinstance = array_map('esc_html', $_POST['instance']);
					...
					if ($form = $this -> Subscribeform() -> find(array('id' => (int) $_POST['form_id']))) {
						...
						if (empty($form -> confirmationtype) || $form -> confirmationtype == "message") {
							...
							if (!empty($form -> confirmation_redirect)) {
								$redirect = do_shortcode(wp_unslash(__($form -> confirmation_redirect)));
		...
		function ajax_form_preview() {
			...
			if (!empty($id)) {				
				if ($form = $this -> Subscribeform() -> find(array('id' => $id))) {	
					...
					echo '<p class="newsletters_error">' . __('Form could not be loaded', 'wp-mailinglist') . '</p>';
				}
			} else {
				echo '<p class="newsletters_error">' . __('No form was specified', 'wp-mailinglist') . '</p>';
			}
		...
		function ajax_getlistfields() {
			...
			$widget_id = esc_html($_GET['widget_id']);
			$instance = map_deep($_POST['instance'], 'sanitize_text_field');
			...
			jQuery(document).ready(function() {
				...
				jQuery('#<?php echo $widget_id; ?>-form .newsletters-list-checkbox').on('click', function() { newsletters_refreshfields('<?php echo esc_html($widget_id); ?>'); });
				jQuery('#<?php echo $widget_id; ?>-form .newsletters-list-select').on('change', function() { newsletters_refreshfields('<?php echo esc_html($widget_id); ?>'); });
			});
			</script>
		...
		function ajax_mailapi_mailgun_action() {
			...
			if (!empty($_POST['ac'])) {
				$mailgun_apikey = sanitize_text_field($_POST['key']);
				$mailgun_domain = sanitize_text_field($_POST['domain']);
				...
				$mg = $mailgun -> create($mailgun_apikey, $mailgun_region);
		...
		function ajax_mailapi_amazonses_action() {
			...
			if (!empty($_REQUEST['ac'])) {				
				require($this -> plugin_base() . DS . 'vendor' . DS . 'autoload.php');
				$ses = new SimpleEmailService(sanitize_text_field($_REQUEST['key']), sanitize_text_field($_REQUEST['secret']), 'email.' . sanitize_text_field($_REQUEST['region']) . '.amazonaws.com', false);
		...
		function ajax_autocomplete_histories() {
			...
			$conditions = array();
			if (!empty($_REQUEST['q'])) {
				$conditions['subject'] = "LIKE '%" . esc_sql($_REQUEST['q']) . "%'";
			}
		...
		function ajax_deletecontentarea() {
			...
			if (!empty($_POST['number']) && !empty($_POST['history_id'])) {
				$this -> Content() -> delete_all(array('number' => sanitize_text_field($_POST['number']), 'history_id' => sanitize_text_field($_POST['history_id'])));
			}
		...
		function ajax_newsletters_autosave_blockeditor() {
			...
			$post_data = $_POST;
			$post_id = $post_data['ID'] = (!empty($_POST['post_ID'])) ? (int) sanitize_text_field($_POST['post_ID']) : false;
			...
			if (!empty($history_id)) {
				...
				<iframe width="100%" style="width:100%;" frameborder="0" scrolling="no" class="autoHeight widefat" src="<?php echo admin_url('admin-ajax.php?action=newsletters_gauge&value=' . $score . '&security=' . wp_create_nonce('gauge')); ?>"></iframe>
	    ...
	    function ajax_spamscore_blockeditor() {
			...
			$message = $this -> render_email('send', array('message' => $content, 'subject' => $subject, 'subscriber' => $subscriber, 'history_id' => $history_id), false, true, true, $_POST['newsletters_theme_id']);
			$eunique = $Html -> eunique($subscriber, $history_id);
			$this -> execute_mail($subscriber, false, $subject, $message, $history -> attachments, $history_id, $eunique, false);			
			$spamscore = $this -> spam_score($newsletters_emailraw, "long");
	    ...
	    function ajax_spamscorerunner() {
		    ...
		    if (current_user_can('newsletters_send')) {
		    	...
		    	$subject = wp_kses_post(wp_unslash($_POST['subject']));
		    	$content = wp_kses_post(wp_unslash($_POST['content']));
		    	$history_id = sanitize_text_field($_POST['ishistory']);
				$message = $this -> render_email('send', array('message' => $content, 'subject' => $subject, 'subscriber' => $subscriber, 'history_id' => $history_id), false, true, true, $_POST['theme_id']);
				$eunique = $Html -> eunique($subscriber, $history_id);
				$this -> execute_mail($subscriber, false, $subject, $message, $history -> attachments, $history_id, $eunique, false);
				$spamscore = $this -> spam_score($newsletters_emailraw, "long");
	    ...
	    function ajax_gauge() {
		    ...
		    if (current_user_can('newsletters_send')) {
		    	$value = (empty($_REQUEST['value'])) ? 0 : sanitize_text_field($_REQUEST['value']);
		...
		function ajax_historyiframe($returnoutput = false, $text = false, $history_id = false, $ajax = true) {
			...
			$id = (empty($history_id)) ? esc_html($_GET['id']) : $history_id;				
			$email = $this -> History() -> find(array('id' => $id));
				...
				if (!empty($_GET['delete'])) {
					...
					if (!empty($_POST)) {
						if (empty($_REQUEST['serialkey'])) { $errors[] = __('Please fill in a serial key.', 'wp-mailinglist'); }
							...
							$this -> update_option('serialkey', esc_html($_REQUEST['serialkey']));	//update the DB option
							$this -> delete_all_cache('all');
					...
					if (!empty($_POST)) {
						...
						if (!empty($_POST['password1'])) {
							...
							elseif ($_POST['password1'] != $_POST['password2']) { $errors['password'] = __('Passwords do not match', 'wp-mailinglist'); }
								...
								$password = sanitize_text_field($_POST['password1']);
		...
		function ajax_subscribercountdisplay() {			
			...
			if (!empty($subscribercount_query)) {
				...
				$perpage = 50;
				$start = (empty($_GET['start'])) ? 0 : esc_html($_GET['start']);
				$realstart = ($start + 1);
				$end = ($start + $perpage);
				...
				if ($subscribers = $wpdb -> get_results($subscribercount_query)) {
					...
					<?php if ($total_subscribers > $end) : ?>
						<a href="" class="button button-secondary" onclick="jQuery.colorbox({width:'80%', height:'80%', title:'<?php _e('Selected Subscribers', 'wp-mailinglist'); ?>', href:'<?php echo admin_url('admin-ajax.php?action=newsletters_subscribercountdisplay&security=' . wp_create_nonce('subscribercountdisplay') . '&start=' . $nextstart); ?>'}); return false;"><?php _e('Next Page', 'wp-mailinglist'); ?> <i class="fa fa-caret-right"></i></a>
					<?php endif; ?>
					</p>
		...
		function ajax_subscribercount($count = true) {
			...
			$status = (empty($_POST['status'])) ? 'active' : sanitize_text_field($_POST['status']);
			...
			if (!empty($_POST['mailinglists'])) {
				...
				if (!empty($_POST['dofieldsconditions'])) {
					...
					$condquery = sanitize_text_field($_POST['condquery']);
				...
				if (!empty($_POST['daterange']) && $_POST['daterange'] == "Y") {
					if (!empty($_POST['daterangefrom']) && !empty($_POST['daterangeto'])) {
						$daterangefrom = date_i18n("Y-m-d H:i:s", strtotime(sanitize_text_field($_POST['daterangefrom'])));
						$daterangeto = date_i18n("Y-m-d H:i:s", strtotime(sanitize_text_field($_POST['daterangeto'])));
				...
				if (!empty($_POST['countries'])) {
					if (!empty($_POST['selectedcountries']) && is_array($_POST['selectedcountries'])) {
						$countries = implode("', '", map_deep($_POST['selectedcountries'], 'sanitize_text_field'));
		...
		function sc_management($atts = array(), $content = null) {
			...
			switch ($method) {
				case 'paidsubscription'			:
					...
					$extend = sanitize_text_field($_REQUEST['extend']);
				...
				case 'resubscribe'			:
					...
					if (!empty($_GET['email']) && (!empty($_GET['mailinglists']))) {
						...
						$subscriber['email'] = esc_html($_GET['email']);
						$subscriber['mailinglists'] = explode(",", esc_html($_GET['mailinglists']));
						$subscriber['active'] = "Y";
		...
		function enqueue_scripts() {
			...
			if (is_admin() && !defined('DOING_AJAX')) {
				...
				if ($screen -> id == "dashboard" || preg_match("/(index.php)/si", $_SERVER['REQUEST_URI']) ||
					...
					wp_enqueue_script('chartjs', $this -> render_url('js/chartjs/Chart.js', 'admin', false), array('jquery'), '2.8.0', false);
				}
				...
				if ((!empty($_GET['page']) && in_array($_GET['page'], (array) $this -> sections)) || preg_match("/(post\.php|post\-new\.php)/", $_SERVER['REQUEST_URI'], $matches)) {
					...
					if (empty($matches)) {
						wp_enqueue_script('ckeditor', $this -> render_url('js/ckeditor/ckeditor.js', 'admin', false), array('jquery'), '4.4.7', false);
				...
				if ($field = $Field -> get($field_id)) {
					...
					if (!empty($form_id) && !empty($form_field)) {
						...
						if (!empty($form_styling['fieldshowlabel'])) {
							echo '<label for="' . ((!empty($list) && $list == "checkboxes") ? 'listsselectall' : '') . '" class="control-label' . ((!empty($list) && $list == "checkboxes") ? ' form-inline' : '') . ' ' . $this -> pre . 'customfield ' . $this -> pre . 'customfield' . $field_id . '">' . ((!empty($list) && $list == "checkboxes") ? $listsselectall . ' ' : '') . ((empty($form_field -> label)) ? __($field -> title) : __($form_field -> label)) . ((empty($form_field -> required)) ? ' <small class="small text-muted">' . __('(optional)', 'wp-mailinglist') . '</small>' : '') . '</label> ';
						...
						$placeholder =  ' placeholder="' . ((!empty($field -> watermark) && $watermark == true && empty($offsite)) ? esc_attr(wp_unslash(__($field -> watermark))) : '') . '"';
					...
					if (!empty($Subscriber -> data[$field -> slug])) {
						...
						$fieldvalue = esc_html($_REQUEST[$field -> slug]);
					}
					...
					switch ($field -> type) {
						case 'hidden'			:
							...
							break;
						case 'text'				:						
							echo '<input' . $placeholder . ' class="form-control ' . $this -> pre . ' ' . $this -> pre . 'text' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : '') . '" id="' . $this -> pre . '-' . $optinid . '' . $field -> slug . '" ' . $Html -> tabindex($optinid) . ' type="text" name="' . $fieldname . '" value="' . esc_attr(wp_unslash($fieldvalue)) . '" />';
							break;
						case 'special'			:
							switch ($field -> slug) {
								case 'list'				:
						...
						case 'select'			:						
							...
							if (!empty($field -> newfieldoptions)) {
								foreach ($field -> newfieldoptions as $option_id => $option_value) {
									$select = (!empty($fieldvalue) && ($fieldvalue == $option_id || $fieldvalue == __($option_value))) ? 'selected="selected"' : '';
						...
						case 'radio'			:
							if (!empty($field -> newfieldoptions)) {
								foreach ($field -> newfieldoptions as $option_id => $option_value) {
									$checked = ($fieldvalue == $option_id || (!empty($fieldvalue) && $fieldvalue == __($value))) ? 'checked="checked"' : '';
						...
						case 'file'				:
							...
							<label class="btn btn-secondary btn-file">
								<?php _e('Browse...', 'wp-mailinglist'); ?> <input onchange="jQuery('#file_upload_<?php echo $field -> id; ?>_info').html(jQuery(this).val().replace('C:\\fakepath\\', ''));" type="file" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo esc_attr(wp_unslash($fieldname)); ?>" id="file_upload_<?php echo $field -> id; ?>" />
							</label>
							...
							if ($countries = $this -> Country() -> select()) {
								...
								<select class="<?php echo $this -> pre; ?>country <?php echo $this -> pre; ?> <?php echo $this -> pre; ?>precountry<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" id="<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo $fieldname; ?>">
									...
									<?php foreach ($countries as $id => $value) : ?>
										<option <?php echo (!empty($fieldvalue) && $fieldvalue == $id) ? 'selected="selected"' : ''; ?> value="<?php echo esc_attr($id); ?>"><?php echo $value; ?></option>
									<?php endforeach; ?>
								</select>
						...
						case 'pre_date'			:
							...
							<input type="text" class="<?php echo $this -> pre; ?>predate <?php echo $this -> pre; ?>text <?php echo $this -> pre; ?> <?php echo $this -> pre; ?>predate<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" value="<?php echo esc_attr(wp_unslash($currentDate)); ?>" name="<?php echo $fieldname; ?>" id="<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>" />
						...
						case 'pre_gender'		:
							...
							<select<?php echo $placeholder; ?> class="<?php echo $this -> pre; ?> <?php echo $this -> pre; ?>pregender<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" style="width:auto;" id="<?php echo $this -> pre; ?>-<?php echo $optinid; ?><?php echo $field -> slug; ?>" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo $fieldname; ?>">
								...
								<option <?php echo (!empty($fieldvalue) && $fieldvalue == "male") ? 'selected="selected"' : ''; ?> value="male"><?php _e('Male', 'wp-mailinglist'); ?></option>
								<option <?php echo (!empty($fieldvalue) && $fieldvalue == "female") ? 'selected="selected"' : ''; ?> value="female"><?php _e('Female', 'wp-mailinglist'); ?></option>
								<option <?php echo (!empty($fieldvalue) && $fieldvalue == "other") ? 'selected="selected"' : ''; ?> value="other"><?php _e('Other', 'wp-mailinglist'); ?></option>
							</select>
					...
					if (!empty($errors[$field -> slug])) {
						if (empty($form) || (!empty($form) && !empty($form_styling['fielderrors']))) {
							...
							<div id="newsletters-<?php echo $optinid; ?>-<?php echo $field -> slug; ?>-error" class="newsletters-field-error alert alert-danger my-1 ui-state-error ui-corner-all">
								<i class="fa fa-exclamation-triangle"></i> <?php echo wp_unslash($errors[$field -> slug]); ?>
							</div>
					...
					if (!empty($field -> type) && $field -> type == "file") {
						...
						if (!empty($fieldvalue)) {
							echo $Html -> file_custom_field($fieldvalue, $field -> filesizelimit, $filetypes, $field, true);
		...
		function gen_mailinglist_names($subscriber = null) {
			...
			if (empty($subscriber -> mailinglists)) {
				if (!empty($_POST['mailinglists'])) {
					$subscriber -> mailinglists = map_deep($_POST['mailinglists'], 'sanitize_text_field');
				} else {
		...
		function gen_unsubscribe_comments() {
			...
			if (!empty($_POST[$this -> pre . 'comments'])) {
				...
				$unsubscribecomments .= "------------------------------------" . "\r\n";
				$unsubscribecomments .= esc_html(wp_unslash($_POST[$this -> pre . 'comments'])) . "\r\n";
				$unsubscribecomments .= "------------------------------------" . "\r\n";
			}
		...
		function subscription_confirm($subscriber = array()) {			
			...
			if (!empty($subscriber)) {
				if (!empty($_POST['list_id'])) {
					$subscriber -> mailinglists = map_deep($_POST['list_id'], 'sanitize_text_field');
					...
					if ($mailtype == "smtp" || $mailtype == "gmail" || (!empty($newsletters_presend) && $newsletters_presend == true)) {
						...
						try {
							...
							if (!empty($newsletters_presend) && $newsletters_presend == true) {
								...
								$yourmailexchange = $_SERVER['HTTP_HOST'];								
								...
								$receivermailexchange = $_SERVER['HTTP_HOST'];
		...
		function ajax_load_new_editor() {
			...
			<div class="postbox" id="contentareabox<?php echo esc_attr($_REQUEST['contentarea']); ?>">
				<div class="handlediv" title="Click to toggle"><br></div>
					<h3 class="hndle"><span><?php echo __('Content Area', 'wp-mailinglist'); ?> <?php echo esc_html($_REQUEST['contentarea']); ?></span></h3>
					<div class="inside">
					...
					<table id="post-status-info" cellpadding="0" cellspacing="0">
						<tbody>
							<tr>
								<td id="wp-word-count">
									<span id="word-count"><code>[newsletters_content id="<?php echo esc_attr($_REQUEST['contentarea']); ?>"]</code></span>
								</td>
					...
					<p><a href="" onclick="if (confirm('<?php echo __('Are you sure you want to remove this content area?', 'wp-mailinglist'); ?>')) { deletecontentarea('<?php echo esc_attr($_REQUEST['contentarea']); ?>', ''); } return false;" class="button button-secondary"><?php _e('Delete', 'wp-mailinglist'); ?></a></p>
				</div>
			</div>
		...
		function ajax_delete_lps_post() {
			...
			if (!empty($_POST['id'])) {
				$this -> Latestpost() -> delete(sanitize_text_field($_POST['id']));
			}
		...
		function updating_plugin() {
			...
			if (version_compare($this -> version, $cur_version) === 1) {
				...
				if (version_compare($cur_version, "4.6.19") < 0) {
					$this -> update_options();
					$this -> check_tables();
					$this -> initialize_default_themes();
