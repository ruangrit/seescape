...
<form action="<?php echo $action; ?>" method="post" id="<?php echo $widget_id; ?>-form" class="newsletters-form" enctype="multipart/form-data">
	...
	<div id="<?php echo $widget_id; ?>-fields" class="newsletters-form-fields">
		<?php $list_id = (empty($_POST['list_id'])) ? $instance['list'] : sanitize_text_field($_POST['list_id']); ?>
	...
	<?php if ($captcha_type = $this -> use_captcha(__($instance['captcha']))) : ?>		
		...
		<?php elseif ($captcha_type == "recaptcha") : ?>
			...
			<div class="newsletters-captcha-wrapper form-group newsletters-fieldholder <?php echo ($recaptcha_type == "invisible") ? 'newsletters-fieldholder-hidden hidden' : ''; ?>">
				<div id="newsletters-<?php echo $widget_id; ?>-recaptcha-challenge" class="newsletters-recaptcha-challenge my-1"></div>
...
jQuery(document).ready(function() {
	...
	<?php if (!empty($ajax) && $ajax == "Y") : ?>
		...
		if (jQuery.isFunction(jQuery.fn.ajaxForm)) {
			jQuery('#<?php echo $widget_id; ?>-form').ajaxForm({
				url: newsletters_ajaxurl + 'action=wpmlsubscribe&widget=<?php echo $widget; ?>&widget_id=<?php echo $widget_id; ?>&number=<?php echo $number; ?>&security=<?php echo wp_create_nonce('subscribe'); ?>',
	...
	$recaptcha_element = jQuery('#newsletters-<?php echo $widget_id; ?>-recaptcha-challenge');
	...
	$form = $postpage.find('#<?php echo $widget_id; ?>-form');
