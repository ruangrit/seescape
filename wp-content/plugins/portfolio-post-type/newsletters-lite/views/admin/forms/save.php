		...
		<?php if (!empty($styling)) : ?>
			<?php foreach ($styling as $skey => $sval) : ?>
				<?php if ($skey != "twocolumns") : ?>
					<input type="hidden" name="styling[<?php echo $skey; ?>]" value="<?php echo esc_attr(wp_unslash($sval)); ?>" />
				<?php endif; ?>
			<?php endforeach; ?>
		<?php endif; ?>
								...
								<div id="title-tabs">
									...
									<?php foreach ($languages as $language) : ?>
										<div id="title-tabs-<?php echo $language; ?>">
											<input type="text" class="widefat title" name="title[<?php echo $language; ?>]" value="<?php echo esc_attr(wp_unslash($this -> language_use($language, $form -> title))); ?>" id="title_<?php echo $language; ?>" placeholder="<?php echo __('Enter form name here', 'wp-mailinglist'); ?>" />
										</div>
									<?php endforeach; ?>
								</div>
							...
							<div class="inside">
								<div id="edit-slug-box">
									...
									<span id="sample-permalink">
										...
										<a href="<?php echo admin_url('admin.php?page=' . $this -> sections -> forms . '&method=codes&id=' . $form -> id); ?>"><?php _e('More embedding options', 'wp-mailinglist'); ?></a>
									</span>
								</div>
							</div>
						<?php endif; ?>
						<p>
							<label style="cursor: pointer; font-weight:bold;"><input onclick="jQuery('#postbox-container-2').toggleClass('newsletters-two-columns');" <?php echo (!empty($styling['twocolumns'])) ? 'checked="checked"' : ''; ?> type="checkbox" name="styling[twocolumns]" value="1" /> <?php _e('Use two columns inside posts/pages', 'wp-mailinglist'); ?></label>
						</p>
...
function newsletters_forms_field_delete(field_id) {
	...
	jQuery.ajax({
		url: newsletters_ajaxurl + "action=newsletters_forms_deletefield&security=<?php echo wp_create_nonce('forms_deletefield'); ?>",
		method: "POST",
...
function newsletters_forms_field_add(element, target) {
	...
	jQuery.ajax({
		url: newsletters_ajaxurl + 'action=newsletters_forms_addfield&security=<?php echo wp_create_nonce('forms_addfield'); ?>',
		method: "POST",
		data: {
			id: field_id,
			type: field_type,
