			...
			<?php foreach ($latestpostssubscriptions as $latestpostssubscription) : ?>
				<tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>" id="latestposts_row_<?php echo $latestpostssubscription -> id; ?>">
					...
					<td>
						...
						<a href="" onclick="jQuery.colorbox({href:newsletters_ajaxurl + 'action=newsletters_lpsposts&id=<?php echo $latestpostssubscription -> id; ?>&security=<?php echo wp_create_nonce('lpsposts'); ?>'}); return false;" id="latestposts_history_count_<?php echo esc_attr(wp_unslash($latestpostssubscription -> id)); ?>"><?php echo sprintf(__('%s posts used/sent', 'wp-mailinglist'), $posts_used); ?></a>
						<a onclick="if (confirm('<?php _e('Are you sure you want to clear the posts history for this latest posts subscription?', 'wp-mailinglist'); ?>')) { latestposts_clearhistory('<?php echo esc_attr(wp_unslash($latestpostssubscription -> id)); ?>'); } return false;" href="<?php echo admin_url('admin.php?page=' . $this -> sections -> settings . '&amp;method=clearlpshistory&id=' . $latestpostssubscription -> id); ?>" class=""><i class="fa fa-trash"></i></a>
...
function latestposts_changestatus(id, status) {
	...
	jQuery.ajax({
		type: "POST",
		url: newsletters_ajaxurl + 'action=newsletters_latestposts_changestatus&security=<?php echo wp_create_nonce('latestposts_changestatus'); ?>',
		data: {
			id:id, 
			status:status
		}
...
function latestposts_del_row(id) {
	...
	jQuery.post(newsletters_ajaxurl + 'action=newsletters_latestposts_delete&id=' + id + '&security=<?php echo wp_create_nonce('latestposts_delete'); ?>', false, function(response) {
...
function latestposts_clearhistory(id) {
	...
	jQuery.ajax({
		type: "POST",
		url: newsletters_ajaxurl + 'action=newsletters_latestposts_clearhistory&security=<?php echo wp_create_nonce('latestposts_clearhistory'); ?>',
		data: {
			id: id
		}
