...
<table class="form-table">
	<tbody>
		<tr>
			...
			<td>
				<input type="text" class="widefat" id="<?php echo $this -> pre; ?>adminemail" name="adminemail" value="<?php echo esc_attr(wp_unslash($this -> get_option('adminemail'))); ?>" />
		...
		<tr>
			...
			<td>
					...
					<div id="smtpfromnametabs">
			            ...
			            <?php foreach ($languages as $language) : ?>
			            	<div id="smtpfromnametab<?php echo $tabnumber; ?>">
			            		<input type="text" name="smtpfromname[<?php echo $language; ?>]" value="<?php echo esc_attr(wp_unslash($this -> language_use($language, $smtpfromname))); ?>" id="smtpfromname_<?php echo $language; ?>" class="widefat" />
			            	</div>
			            	<?php $tabnumber++; ?>
			            <?php endforeach; ?>
					</div>
				...
				<?php else : ?>
            		<input class="widefat" type="text" id="<?php echo $this -> pre; ?>smtpfromname" name="smtpfromname" value="<?php echo esc_attr(wp_unslash($this -> get_option('smtpfromname'))); ?>" />
            	<?php endif; ?>
					...
					<div id="smtpfromtabs">
			            ...
			            <?php foreach ($languages as $language) : ?>
			            	<div id="smtpfromtab<?php echo $tabnumber; ?>">
			            		<input type="text" name="smtpfrom[<?php echo $language; ?>]" value="<?php echo esc_attr(wp_unslash($this -> language_use($language, $smtpfrom))); ?>" id="smtpfrom_<?php echo $language; ?>" class="widefat" />
			            	</div>
			            	<?php $tabnumber++; ?>
			            <?php endforeach; ?>
					</div>
...
<table class="form-table">
	<tbody>
		<tr class="advanced-setting">
			<th><label for="bccemails"><?php _e('BCC Outgoing Emails', 'wp-mailinglist'); ?></label></th>
			<td>
				<label><input <?php checked($bccemails, 1, true); ?> onclick="if (jQuery(this).is(':checked')) { jQuery('#bccemails_div').show(); } else { jQuery('#bccemails_div').hide(); }" type="checkbox" name="bccemails" id="bccemails" value="1" /> <?php _e('Yes, BCC an email address on all outgoing emails.', 'wp-mailinglist'); ?></label>
			</td>
		</tr>
	</tbody>
</table>
...
<div class="newsletters_indented" id="bccemails_div" style="display:<?php echo (!empty($bccemails)) ? 'block' : 'none'; ?>;">
	<table class="form-table">
		<tbody>
			<tr>
				<th><label for="bccemails_address"><?php _e('BCC Email Address', 'wp-mailinglist'); ?></label></th>
				<td>
					...
					<span class="howto"><?php _e('Fill in a valid email address to BCC on all outgoing emails.', 'wp-mailinglist'); ?></span>
				</td>
			</tr>
		</tbody>
	</table>
...
<div class="newsletters_indented" id="mailtypediv" style="display:<?php echo $mailtypedisplay = ($mailtype_current == "smtp" || $mailtype_current == "gmail") ? 'block' : 'none'; ?>;">
	<table class="form-table">
		<tbody>
			<tr>
				...
				<td>
                	<input class="widefat" type="text" id="<?php echo $this -> pre; ?>smtphost" name="smtphost" value="<?php echo esc_attr(wp_unslash($this -> get_option('smtphost'))); ?>" />
			...
			<tr>
				...
				<td>
                	<input class="widefat" style="width:65px;" type="text" name="smtpport" value="<?php echo esc_attr(wp_unslash($this -> get_option('smtpport'))); ?>" id="<?php echo $this -> pre; ?>smtpport" />
	...
	<div class="newsletters_indented" id="smtpauthdiv" style="display:<?php echo $authdisplay; ?>;">
		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="<?php echo $this -> pre; ?>smtpuser"><?php _e('SMTP Username', 'wp-mailinglist'); ?></label></th>
					<td><input autocomplete="off" class="widefat" type="text" id="<?php echo $this -> pre; ?>smtpuser" name="smtpuser" value="<?php echo esc_attr(wp_unslash($this -> get_option('smtpuser', false))); ?>" /></td>
				</tr>
				<tr>
					<th><label for="<?php echo $this -> pre; ?>smtppass"><?php _e('SMTP Password', 'wp-mailinglist'); ?></label></th>
					<td><input autocomplete="off" class="widefat" type="password" id="<?php echo $this -> pre; ?>smtppass" name="smtppass" value="<?php echo esc_attr(wp_unslash($this -> get_option('smtppass', false))); ?>" /></td>
				</tr>
			</tbody>
		</table>
	</div>
...
<div class="newsletters_indented" id="mailtypeapi" style="display:<?php echo (!empty($mailtype_current) && $mailtype_current == "api") ? 'block' : 'none'; ?>;">
	...
	<div class="newsletters_indented" id="mailapidiv_mailgun" style="display:<?php echo (!empty($mailapi) && $mailapi == "mailgun") ? 'block' : 'none'; ?>">
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					...
					<td>
						<input class="widefat" type="text" placeholder="<?php echo esc_attr(wp_unslash($_SERVER['HTTP_HOST'])); ?>" name="mailapi_mailgun_domain" value="<?php echo esc_attr(wp_unslash($mailgun_domain)); ?>" id="mailapi_mailgun_domain" />
						<span class="howto"><?php _e('Verified MailGun sending domain, the domain in your From Address.', 'wp-mailinglist'); ?></span>
					</td>
				</tr>
				<tr>
					<th><label for="mailapi_mailgun_region"><?php _e('MailGun Region', 'wp-mailinglist'); ?></label></th>
					<td>
						<select name="mailapi_mailgun_region" id="mailapi_mailgun_region">
							<option <?php echo (empty($mailgun_region) || $mailgun_region == "US") ? 'selected="selected"' : ''; ?> value="US"><?php _e('US Region', 'wp-mailinglist'); ?></option>
							<option <?php echo (!empty($mailgun_region) && $mailgun_region == "EU") ? 'selected="selected"' : ''; ?> value="EU"><?php _e('EU Region', 'wp-mailinglist'); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th><label for="mailapi_mailgun_emailvalidation"><?php _e('MailGun Email Validation', 'wp-mailinglist'); ?></label></th>
					<td>
						<label><input onclick="if (jQuery(this).is(':checked')) { jQuery('#mailapi_mailgun_emailvalidation_div').show(); } else { jQuery('#mailapi_mailgun_emailvalidation_div').hide(); }" <?php echo (!empty($mailapi_mailgun_emailvalidation)) ? 'checked="checked"' : ''; ?> type="checkbox" name="mailapi_mailgun_emailvalidation" value="1" id="mailapi_mailgun_emailvalidation" /> <?php _e('Use MailGun email validation API to validate email addresses.', 'wp-mailinglist'); ?></label>
						<span class="howto"><?php _e('Will be used for subscribe forms, new add by admin, imports, etc. if turned on.', 'wp-mailinglist'); ?></span>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<div id="mailapi_mailgun_emailvalidation_div" style="display:<?php echo (!empty($mailapi_mailgun_emailvalidation)) ? 'block' : 'none'; ?>;">
			<p class="newsletters_warning"><?php _e('This only works with MailGun paid accounts and NOT free accounts.', 'wp-mailinglist'); ?></p>
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="mailapi_mailgun_pubapikey"><?php _e('Public API Key', 'wp-mailinglist'); ?></label></th>
						<td>
							...
							<span class="howto"><?php _e('Get your public API key in your Mailgun dashboard.', 'wp-mailinglist'); ?></span>
						</td>
					</tr>
				</tbody>
			</table>
				...
				<tr>
					...
					<td>						
						...
						jQuery('.mailgun-action').on('click', function(e) {
							...
							jQuery.ajax({
								method: "POST",
								url: newsletters_ajaxurl + 'action=newsletters_mailapi_mailgun_action&security=<?php echo wp_create_nonce('mailapi_mailgun_action'); ?>',
								data: {
									ac: action,
				...
				<tr>
					...
					<td>
						...
						jQuery('.mailapi-mandrill-keytest').on('click', function() {
							...
							jQuery.ajax({
								method: "POST",
								url: newsletters_ajaxurl + 'action=newsletters_mailapi_mandrill_keytest&security=<?php echo wp_create_nonce('mailapi_mandrill_keytest'); ?>',
								data: {
				...
				<tr>
					...
					<td>						
						...
						jQuery('.amazonses-action').on('click', function(e) {
							...
							jQuery.ajax({
								method: "POST",
								url: newsletters_ajaxurl + 'action=newsletters_mailapi_amazonses_action&security=<?php echo wp_create_nonce('mailapi_amazonses_action'); ?>',
								data: {
									ac: action,
		...
		<table class="form-table">
			<tbody>
				<tr>
					...
					<td>
						<textarea id="dkim_private" name="dkim_private" rows="4" cols="100%" class="widefat"><?php echo wp_unslash($private); ?></textarea>
					</td>
				</tr>
			</tbody>
		</table>
...
function testsettings() {
	...
	jQuery.post(newsletters_ajaxurl + 'action=<?php echo $this -> pre; ?>testsettings&security=<?php echo wp_create_nonce('testsettings'); ?>&init=1', formvalues, function(response) {
...
function dkimwizard(formvalues) {
	...
	jQuery.post(newsletters_ajaxurl + 'action=<?php echo $this -> pre; ?>dkimwizard&security=<?php echo wp_create_nonce('dkimwizard'); ?>', formvalues, function(response) {
