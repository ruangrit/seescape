...
$p_id = (empty($_POST['p_id'])) ? $imagespost : sanitize_text_field($_POST['p_id']);
$ID = $p_id;
$post_ID = $p_id;
...
<div class="wrap <?php echo $this -> pre; ?> <?php echo $this -> sections -> send; ?> newsletters">
	...
	<form action="<?php echo admin_url('admin.php?page=' . $this -> sections -> send); ?>" method="post" id="post" name="post" enctype="multipart/form-data">
		...
		<input type="hidden" id="ishistory" name="ishistory" value="<?php echo esc_attr($_POST['ishistory']); ?>" />
		<input type="hidden" id="p_id" name="p_id" value="<?php echo esc_attr($_POST['p_id']); ?>" />
		<input type="hidden" name="inctemplate" value="<?php echo esc_attr($_POST['inctemplate']); ?>" />
		<input type="hidden" name="recurringsent" value="<?php echo esc_attr(wp_unslash($_POST['sendrecurringsent'])); ?>" />
		<input type="hidden" name="post_id" value="<?php echo esc_attr(wp_unslash($_POST['post_id'])); ?>" />
				...
				<div id="post-body-content">
					<div id="titlediv">
						<div id="titlewrap">
							...
							<input onclick="jQuery('iframe#content_ifr').attr('tabindex', '2');" tabindex="1" id="title" autocomplete="off" type="text" placeholder="<?php echo esc_attr(wp_unslash(__('Enter email subject here', 'wp-mailinglist'))); ?>" name="subject" value="<?php echo esc_attr(wp_unslash($_POST['subject'])); ?>" />
					...
					<div id="<?php echo (user_can_richedit()) ? 'postdivrich' : 'postdiv'; ?>" class="postarea edit-form-section" style="position:relative;>;">
						...
						<?php if (version_compare(get_bloginfo('version'), "3.3") >= 0) : ?>
							<?php wp_editor(wp_unslash($_POST['content']), 'content', array('tabindex' => "2", 'tinymce' => $tinymce)); ?>
						<?php else : ?>
							<?php the_editor(wp_unslash($_POST['content']), 'content', 'title', true, 2); ?>
						<?php endif; ?>
...
<?php $history_id = (empty($_POST['ishistory'])) ? sanitize_text_field($_GET['id']) : sanitize_text_field($_POST['ishistory']); ?>
...
function deletecontentarea(number, history_id) {
	if (history_id != "") {
		...
		jQuery.post(newsletters_ajaxurl + 'action=newsletters_deletecontentarea&security=<?php echo wp_create_nonce('deletecontentarea'); ?>', data, function(response) {
...
function newsletters_autosave() {	
	...
	newsletters_autosave_request = jQuery.ajax({
		...
		data: formvalues,
		dataType: "json",
		url: newsletters_ajaxurl + 'action=newsletters_autosave&security=<?php echo wp_create_nonce('newsletters_autosave'); ?>',
		type: "POST",
