			...
			<tbody>
				...
				<tr>
					...
					<td>
								...
								<?php foreach ($mailinglists as $list_id => $list_title) : ?>
									...
									<?php if (!empty($mailinglist -> paid) && $mailinglist -> paid == "Y") : ?>
										<div id="mailinglist_<?php echo $list_id; ?>_expiration" style="display:<?php echo (!empty($Subscriber -> data -> mailinglists) && in_array($list_id, $Subscriber -> data -> mailinglists)) ? 'block' : 'none'; ?>;">
											...
											<input type="text" name="Subscriber[listexpirations][<?php echo $list_id; ?>]" value="<?php echo esc_attr(wp_unslash($Mailinglist -> gen_expiration_date($Subscriber -> data -> id, $list_id))); ?>" id="" />
						...
						<?php endif; ?>
						<p><a href="#" class="button" onclick="jQuery.colorbox({title:'<?php echo esc_attr(wp_unslash(__('Add a Mailing List', 'wp-mailinglist'))); ?>', href:newsletters_ajaxurl + 'action=newsletters_mailinglist_save&security=<?php echo wp_create_nonce('mailinglist_save'); ?>&fielddiv=newsletters-mailinglists-checkboxes&fieldname=Subscriber[mailinglists]'}); return false;"><i class="fa fa-plus-circle"></i> <?php _e('Add Mailing List', 'wp-mailinglist'); ?></a></p>
