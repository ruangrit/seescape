									...
									<?php if ($listsselect = $Mailinglist -> select(true, $subscriber -> mailinglists)) : ?>
										<?php foreach ($listsselect as $list_id => $list_title) : ?>
											<label class="<?php echo $this -> pre; ?>checkboxlabel <?php echo $this -> pre; ?>">
												<input checked="checked" class="<?php echo $this -> pre; ?>checkbox" <?php echo (!empty($data['unsubscribelists']) && in_array($list_id, $data['unsubscribelists'])) ? 'checked="checked"' : ''; ?> type="checkbox" name="unsubscribelists[]" value="<?php echo esc_attr(wp_unslash($list_id)); ?>" id="lists_<?php echo $list_id; ?>" /> <?php echo wp_unslash($list_title); ?>
											</label>
										<?php endforeach; ?>
									<?php else : ?>
							...
							<td>
								<?php if (!empty($otherlists)) : ?>
									<?php foreach ($otherlists as $otherlist_id => $otherlist_title) : ?>
										<label class="<?php echo $this -> pre; ?>checkboxlabel <?php echo $this -> pre; ?>">
											<input class="<?php echo $this -> pre; ?>checkbox" <?php echo (!empty($data['unsubscribelists']) && in_array($otherlist_id, $data['unsubscribelists'])) ? 'checked="checked"' : ''; ?> type="checkbox" name="unsubscribelists[]" value="<?php echo esc_attr(wp_unslash($otherlist_id)); ?>" id="lists_<?php echo $otherlist_id; ?>" /> <?php echo wp_unslash($otherlist_title); ?>
										</label>
									<?php endforeach; ?>
								<?php else : ?>
