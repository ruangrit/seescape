	...
	class wpMail extends wpMailPlugin {
		...
		function admin_notices() {
			...
			if (is_admin() && !defined('DOING_AJAX')) {
				...
				if (!$this -> ci_serial_valid() && (empty($page) || $page != $this -> sections -> submitserial)) {
					...
					jQuery(document).ready(function(e) {
		                jQuery('#<?php echo $this -> pre; ?>submitseriallink').click(function() {
							jQuery.colorbox({href:newsletters_ajaxurl + "action=<?php echo $this -> pre; ?>serialkey&security=<?php echo wp_create_nonce('serialkey'); ?>"});
							return false;
						});
		            });
					</script>
				...
				if (!empty($_GET[$this -> pre . 'updated'])) {
					$this -> render_message(wp_unslash(urldecode(sanitize_text_field($_GET[$this -> pre . 'message']))));
				}

				if (!empty($_GET[$this -> pre . 'error'])) {
					$this -> render_error(wp_unslash(urldecode(sanitize_text_field($_GET[$this -> pre . 'message']))));
				}
		...
		function custom_post_types() {			
			...
			$newsletter_args = array(
				...
				'labels'				=>	array(
					'name' 					=> 	__($this -> name, 'wp-mailinglist'), 
		...
		function init_early() {			
			...
			$wpmlmethod = (empty($_POST[$this -> pre . 'method'])) ? null : sanitize_text_field($_POST[$this -> pre . 'method']);
			$method = (empty($_GET[$this -> pre . 'method'])) ? $wpmlmethod : sanitize_text_field($_GET[$this -> pre . 'method']);
		...
		function init() {			
			...
			if (!empty($_GET[$this -> pre . 'link']) || !empty($_GET['newsletters_link'])) {
				$link_hash = (empty($_GET[$this -> pre . 'link'])) ? sanitize_text_field($_GET['newsletters_link']) : sanitize_text_field($_GET[$this -> pre . 'link']);
				
				if ($link = $this -> Link() -> find(array('hash' => $link_hash))) {

					$email_conditions = array('history_id' => sanitize_text_field($_GET['history_id']));
					if (!empty($_GET['subscriber_id'])) { $email_conditions['subscriber_id'] = sanitize_text_field($_GET['subscriber_id']); }
					if (!empty($_GET['user_id'])) { $email_conditions['user_id'] = sanitize_text_field($_GET['user_id']); }
					...
					$click_data = array(
						'link_id'			=>	$link -> id,
						'history_id'		=>	sanitize_text_field($_GET['history_id']),
					...
					case 'newsletter'					:
						...
						if (!empty($id)) {
							if ($email = $this -> History() -> find(array('id' => $id))) {
								$Db -> model = $Subscriber -> model;
								$subscriber = $Subscriber -> get(sanitize_text_field($_GET['subscriber_id']), false);
								...
								$subscriber -> mailinglist_id = sanitize_text_field($_GET['mailinglist_id']);
								...
								$content = $this -> render_email('send', array('print' => sanitize_text_field($_GET['print']), 'message' => $message, 'subject' => $email -> subject, 'subscriber' => $subscriber, 'history_id' => $id), false, true, true, $email -> theme_id);
								$output = "";
								ob_start();
						...
						if (!empty($_POST)) {
							if (!empty($_POST['email'])) {
								if ($Subscriber -> email_validate(esc_html($_POST['email']))) {
									...
									if ($subscriber = $Db -> find(array('email' => esc_html($_POST['email'])))) {
										if ($subscriberauth = $this -> gen_auth($subscriber -> id)) {
											$Authnews -> set_emailcookie(esc_html($_POST['email']));
									...
									if (!empty($_POST['event'])) {
										...
										switch ($_POST['event']) {
											...
											case 'clicked'				:
												...
												break;
											case 'complained'			:
												$this -> bounce(sanitize_text_field($_POST['recipient']), "mailgun", __('Spam complaint', 'wp-mailinglist'), $custom_data -> MessageID);
												break;
											case 'bounced'				:
												$this -> bounce(sanitize_text_field($_POST['recipient']), "mailgun", sanitize_text_field($_POST['code']) . ' - ' . sanitize_text_field($_POST['error']), $custom_data -> MessageID);
												break;
											case 'dropped'				:
												$this -> bounce(sanitize_text_field($_POST['recipient']), "mailgun", sanitize_text_field($_POST['code']) . ' - ' . sanitize_text_field($_POST['description']), $custom_data -> MessageID);
												break;
											case 'unsubscribed'			:
												// subscriber has unsubscribed
												if (!empty($subscriber)) {
			...
			if (!empty($method)) {
				switch ($method) {
					...
					case 'offsite'			:
						...
						$form_id = (empty($_GET['form'])) ? false : sanitize_text_field($_GET['form']);
						$list = (empty($_GET['list'])) ? false : sanitize_text_field($_GET['list']);
								...
								if (!empty($form_id) && $form = $this -> Subscribeform() -> find(array('id' => $form_id))) {
									if (empty($form -> confirmationtype) || $form -> confirmationtype == "message") {
										...
										if (!empty($form -> confirmation_redirect)) {
											$subscriberedirecturl = do_shortcode(wp_unslash(__($form -> confirmation_redirect)));
						...
						if (!empty($form_id) && $form = $this -> Subscribeform() -> find(array('id' => $form_id))) {
							...
							$atts['list'] = sanitize_text_field($_GET['list']);
					...
					case 'optin'			:
						...
						if (!empty($_POST)) {							
							if ($subscriber_id = $Subscriber -> optin($_POST)) {								
								...
								if ($paidlist_id = $Mailinglist -> has_paid_list(sanitize_text_field($_POST['list_id']))) {
								...
								if (empty($_POST['form_id'])) {									
									if ($this -> get_option('subscriberedirect') == "Y") {
										...
										if (!empty($_POST['list_id']) && (!is_array($_POST['list_id']) || count($_POST['list_id']) == 1)) {
											if ($subscribelist = $Mailinglist -> get(sanitize_text_field($_POST['list_id'][0]))) {
												if (!empty($subscribelist -> subredirect)) {
													$subscriberedirecturl = __($subscribelist -> subredirect);
												}
											}
									...
									if ($form = $this -> Subscribeform() -> find(array('id' => sanitize_text_field($_POST['form_id'])))) {
										if (empty($form -> confirmationtype) || $form -> confirmationtype == "message") {
											if (!empty($form -> confirmation_message)) {
					...
					case 'activate'			:										
						...
						if (!empty($_GET[$this -> pre . 'subscriber_id']) && !empty($_GET['authkey'])) {
							...
							if (!empty($_GET[$this -> pre . 'mailinglist_id'])) {
								$mailinglists = @explode(",", sanitize_text_field($_GET[$this -> pre . 'mailinglist_id']));
								$mailinglistsstring = sanitize_text_field($_GET[$this -> pre . 'mailinglist_id']);
							}
						...
						if (!empty($_POST['order_number'])) {
							if ($_POST['credit_card_processed'] == "Y") {
								...
								if ($_POST['demo'] == "Y" && $this -> get_option('tcodemo') == "Y") {
									...
									$ordernumber = sanitize_text_field($_POST['order_number']);
								}
								...
								if ($mykey === $_POST['key']) {
									$subscriberid = sanitize_text_field($_POST['subscriber_id']);
									$subscriber = $Subscriber -> get($subscriberid, false);
									$mailinglist_id = sanitize_text_field($_POST['mailinglist_id']);
						...
						switch ($_GET['type']) {
							case 'mandrill'			:
								if (isset($_POST['mandrill_events'])) {
									$events = json_decode(wp_unslash(sanitize_text_field($_POST['mandrill_events'])));
				...
				if ($page == $this -> sections -> subscribers) {
					...
					if (!empty($_POST['screenoptions'])) {
						if (!empty($_POST['fields']) && is_array($_POST['fields'])) {
							$this -> update_option('screenoptions_subscribers_fields', map_deep($_POST['fields'], 'sanitize_text_field'));
						} else { delete_option($this -> pre . 'screenoptions_subscribers_fields'); }
	
						if (!empty($_POST['custom']) && is_array($_POST['custom'])) {
							$this -> update_option('screenoptions_subscribers_custom', map_deep($_POST['custom'], 'sanitize_text_field'));
		...
		function save_newsletter($post_id = null, $post = null, $onlyupdatehistory = false) {
			...
			$history_data = array(
		    	'from'				=>	sanitize_text_field($_POST['newsletters_from']),
		    	'fromname'			=>	sanitize_text_field($_POST['newsletters_fromname']),
	    		'post_id'			=>	false,
	    		'p_id'				=>	$post_id,
				'subject'			=>	$subject,
				'message'			=>	$content,
				'text'				=>	((!empty($_POST['newsletters_customtexton']) && !empty($_POST['newsletters_customtext'])) ? sanitize_textarea_field($_POST['newsletters_customtext']) : false),
				'theme_id'			=>	sanitize_text_field($_POST['newsletters_theme_id']),
				'condquery'			=>	maybe_serialize(map_deep($_POST['newsletters_condquery'], 'sanitize_text_field')),
				'conditions'		=>	maybe_serialize(map_deep($_POST['newsletters_fields'], 'sanitize_text_field')),
				'conditionsscope'	=>	sanitize_text_field($_POST['newsletters_fieldsconditionsscope']),
				'daterange'			=>	sanitize_text_field($_POST['newsletters_daterange']),
				'daterangefrom'		=>	sanitize_text_field($_POST['newsletters_daterangefrom']),
				'daterangeto'		=>	sanitize_text_field($_POST['newsletters_daterangeto']),
				'countries'			=>	array_map('sanitize_text_field', $_POST['newsletters_countries']),
				'selectedcountries'	=>	maybe_serialize(map_deep($_POST['newsletters_selectedcountries'], 'sanitize_text_field')),
				'mailinglists'		=>	maybe_serialize(map_deep($_POST['newsletters_mailinglists'], 'sanitize_text_field')),
				'groups'			=>	maybe_serialize(map_deep($_POST['newsletters_groups'], 'sanitize_text_field')),
				'roles'				=>	maybe_serialize(map_deep($_POST['newsletters_roles'], 'sanitize_text_field')),
				'senddate'			=>	sanitize_text_field($_POST['senddate']),
				//'scheduled'			=>	$_POST['scheduled'],
				'scheduled'			=>	"N",
				'format'			=>	sanitize_text_field($_POST['newsletters_format']),
			);

			if (!empty($_POST['newsletters_history_id'])) {
				$history_data['id'] = sanitize_text_field($_POST['newsletters_history_id']);
			...
			if (!empty($_POST['newsletters_sendrecurring'])) {
				if (!empty($_POST['newsletters_sendrecurringvalue']) && !empty($_POST['newsletters_sendrecurringinterval']) && !empty($_POST['newsletters_sendrecurringdate'])) {
					...
					$history_data['recurringvalue'] = sanitize_text_field($_POST['newsletters_sendrecurringvalue']);
					$history_data['recurringinterval'] = sanitize_text_field($_POST['newsletters_sendrecurringinterval']);
					$history_data['recurringdate'] = sanitize_text_field($_POST['newsletters_sendrecurringdate']);
					$history_data['recurringlimit'] = sanitize_text_field($_POST['newsletters_sendrecurringlimit']);
				}
			}
			...
			switch ($post_status) {
				...
				case 'publish'					:					
					...
					if (empty($errors)) {
						...
						$defaulttexton = esc_html($_POST['newsletters_defaulttexton']);
						if (!empty($defaulttexton) && !empty($_POST['newsletters_customtext'])) {
							$this -> update_option('defaulttexton', true);
							$this -> update_option('defaulttextversion', sanitize_textarea_field($_POST['newsletters_customtext']));
						} else {
									...
									if (!empty($newsletters_mailinglists)) {
										...
										if (!empty($_POST['newsletters_daterange']) && $_POST['newsletters_daterange'] == "Y") {
											if (!empty($_POST['newsletters_daterangefrom']) && !empty($_POST['newsletters_daterangeto'])) {
												$daterangefrom = date_i18n("Y-m-d", strtotime(sanitize_text_field($_POST['newsletters_daterangefrom'])));
												$daterangeto = date_i18n("Y-m-d", strtotime(sanitize_text_field($_POST['newsletters_daterangeto'])));
										...
										if (!empty($_POST['newsletters_countries'])) {
											if (!empty($_POST['newsletters_selectedcountries']) && is_array($_POST['newsletters_selectedcountries'])) {
												$countries = implode("', '", array_map('sanitize_text_field', map_deep($_POST['newsletters_selectedcountries'], 'sanitize_text_field')));
											...
											foreach ($users as $user) {
												...
												if ($sendingprogress == "N") {																
													$queue_process_data = array(
														...
														'history_id'				=>	$history_id,
														'theme_id'					=>	sanitize_text_field($_POST['newsletters_theme_id']),
														'senddate'					=>	sanitize_text_field($_POST['newsletters_senddate']),
													);
				...
				switch ($post_status) {
					...
					case 'future'					:						
						if (!empty($_POST['newsletters_sendasnewsletter'])) {
							...
							update_post_meta($post_id, '_newsletters_mailinglists', map_deep($_POST['newsletters_mailinglists'], 'sanitize_text_field'));
							update_post_meta($post_id, '_newsletters_showdate', sanitize_text_field($_POST['newsletters_showdate']));
							update_post_meta($post_id, '_newsletters_theme_id', sanitize_text_field($_POST['newsletters_theme_id']));
							update_post_meta($post_id, '_newsletters_language', sanitize_text_field($_POST['newsletters_language']));
							update_post_meta($post_id, '_newsletters_sendonpublishef', sanitize_text_field($_POST['newsletters_sendonpublishef']));
					...
					case 'publish'					:
						...
						if (!empty($_POST['newsletters_sendasnewsletter'])) {
							$newsletters_mailinglists = array_map('sanitize_text_field', $_POST['newsletters_mailinglists']);
							$newsletters_showdate = sanitize_text_field($_POST['newsletters_showdate']);
							$newsletters_theme_id = sanitize_text_field($_POST['newsletters_theme_id']);
							$newsletters_language = sanitize_text_field($_POST['newsletters_language']);
							$newsletters_sendonpublishef = sanitize_text_field($_POST['newsletters_sendonpublishef']);
						} else {							
		...
		function admin_send() {
			...
			if (!empty($_POST['post_id'])) {
				...
				$history_id = sanitize_text_field($_GET['id']);
			...
			if (!empty($_POST['publishpost']) && $_POST['publishpost'] == "Y") {
				$status = (!empty($_POST['post_status'])) ? sanitize_text_field($_POST['post_status']) : 'draft';
				$slug = (!empty($_POST['post_slug'])) ? sanitize_text_field($_POST['post_slug']) : $Html -> sanitize($_POST['subject'], '-');

				$post = array(
					'ID'					=>	$post_id,
					'post_title'			=>	wp_kses_post(wp_unslash($_POST['subject'])),
					'post_content'			=>	$this -> strip_set_variables(wp_kses_post(wp_unslash($_POST['content']))),
					'post_status'			=>	$status,
					'post_name'				=>	$slug,
					'post_category'			=>	map_deep($_POST['cat'], 'sanitize_text_field'),
					'post_type'				=>	((empty($_POST['newsletters_post_type'])) ? 'post' : sanitize_text_field($_POST['newsletters_post_type'])),
					'post_author'			=>	(empty($_POST['post_author'])) ? $user_id : sanitize_text_field($_POST['post_author']),
				);
					...
					if (!empty($_POST['newsletter'])) {
						...
						if (!empty($_POST['sendattachment']) && $_POST['sendattachment'] == "1") {
							...
							if (!empty($_POST['ishistory'])) {
								if ($history = $this -> History() -> find(array('id' => sanitize_text_field($_POST['ishistory'])))) {
									$newattachments = $history -> attachments;
								}
							}

							$newfiles = map_deep($_FILES['attachments'], 'sanitize_text_field');
						...
						$_POST['subject'] = wp_kses_post(wp_unslash($_POST['subject']));
						$_POST['content'] = wp_kses_post(wp_unslash($_POST['content']));
						...
						if (empty($errors)) {
							...
							$defaulttexton = sanitize_text_field($_POST['defaulttexton']);
							if (!empty($defaulttexton) && !empty($_POST['customtext'])) {
								$this -> update_option('defaulttexton', true);
								$this -> update_option('defaulttextversion', sanitize_textarea_field($_POST['customtext']));
							} else {
								$this -> delete_option('defaulttexton');
								$this -> delete_option('defaulttextversion');
							}
										...
										if (!empty($_POST['sendrecurring'])) {
											if (!empty($_POST['sendrecurringvalue']) && !empty($_POST['sendrecurringinterval']) && !empty($_POST['sendrecurringdate'])) {
												...
												/*if (!empty($history_curr) && $_POST['sendrecurringdate'] != $history_curr -> recurringdate) {
													...
													$history_data['recurringdate'] = sanitize_text_field($_POST['sendrecurringdate']);
												//}

												$history_data['recurringlimit'] = sanitize_text_field($_POST['sendrecurringlimit']);
											}
										}
												...
												if (!empty($_POST['mailinglists'])) {
													...
													if (!empty($_POST['daterange']) && $_POST['daterange'] == "Y") {
														if (!empty($_POST['daterangefrom']) && !empty($_POST['daterangeto'])) {
															$daterangefrom = date_i18n("Y-m-d", strtotime(sanitize_text_field($_POST['daterangefrom'])));
															$daterangeto = date_i18n("Y-m-d", strtotime(sanitize_text_field($_POST['daterangeto'])));
															$fieldsquery .= " AND (" . $wpdb -> prefix . $Subscriber -> table . ".created >= '" . esc_sql($daterangefrom) . "' AND " . $wpdb -> prefix . $Subscriber -> table . ".created <= '" . esc_sql($daterangeto) . "')";
														}
													}
								...
								if (!empty($_POST['previewemail'])) {
									...
									foreach ($emails as $email) {
										...
										if (is_email($email)) {
											...
											$subject = wp_kses_post(wp_unslash($_POST['subject']));
											$content = wp_kses_post(wp_unslash($_POST['content']));
											$message = $this -> render_email('send', array('message' => $content, 'subject' => $subject, 'subscriber' => $subscriber, 'history_id' => $history_id), false, true, true, $_POST['theme_id']);
											$eunique = $Html -> eunique($subscriber, $history -> id);
			...
			switch ($method) {
				...
				case 'autoresponderscheduling'			:
					...
					if (!empty($_POST['autoresponderscheduling'])) {
						$this -> update_option('autoresponderscheduling', sanitize_text_field($_POST['autoresponderscheduling']));
			...
			switch ($method) {
				case 'save'			:
					if (!empty($_POST)) {
						...
						if (!empty($_GET['group_id'])) { $Mailinglist -> data -> group_id = esc_html($_GET['group_id']); }
							...
							switch ($_POST['action']) {
								...
								case 'setgroup'			:
									if (!empty($_POST['setgroup_id'])) {
										foreach ($lists as $list_id) {
											$Mailinglist -> save_field('group_id', sanitize_text_field($_POST['setgroup_id']), $list_id);
										}
				...
				case 'offsite'			:
					if (!empty($_GET['listid'])) {
						$this -> update_option('offsitelist', esc_html($_GET['listid']));
					} else {
						$msg_type = 'error';
						$message = __('No mailing list was specified', 'wp-mailinglist');
					}
				...
				case 'offsitewizard'	:
					...
					$code = false;
					$listid = (!empty($_GET['listid'])) ? sanitize_text_field($_GET['listid']) : sanitize_text_field($_POST['list']);
					...
					if (!empty($_POST['subscribe']) && $_POST['subscribe'] == "list" && !empty($listid)) {
						...
						$options['title'] = (empty($_POST['title'])) ? get_option('blogname') : sanitize_text_field($_POST['title']);
						$options['list'] = esc_html($listid);
						$options['button'] = (empty($_POST['button'])) ? __('Subscribe', 'wp-mailinglist') : sanitize_text_field($_POST['button']);
						$options['ajax'] = "Y";
						$options['stylesheet'] = (empty($_POST['stylesheet'])) ? "Y" : sanitize_text_field($_POST['stylesheet']);
			...
			switch ($method) {
				case 'save'			:
					if (!empty($_POST)) {
						...
						$subscriber_id = (int) sanitize_text_field($_POST['Subscriber']['id']);
						if (!empty($subscriber_id)) {
							$subscriber = $Subscriber -> get($subscriber_id);
	            ...
	            case 'bounces'					:
	            	...
	            	if (!empty($_GET['filter'])) {
		            	if (!empty($_GET['history_id'])) {
			            	$conditions_and[$bounces_table . '.history_id'] = "IN (" . implode(",", sanitize_text_field($_GET['history_id'])) . ")";
		            	}
	            	} else {
		            	if (!empty($_GET['history_id'])) {
							$conditions[$bounces_table . '.history_id'] = sanitize_text_field($_GET['history_id']);
						}
	            	}
	            ...
	            case 'unsubscribes'				:
					...
					if (!empty($_GET['history_id'])) {
						$conditions[$unsubscribes_table . '.history_id'] = sanitize_text_field($_GET['history_id']);
					}
				...
				default			:
					...
					if (!empty($_POST['screenoptions'])) {
						...
						if (!empty($_POST['perpage'])) {
							$oldperpage = sanitize_text_field($_POST['perpage']);
						}
					}
					...
					if (!empty($newsletters_filter_subscribers)) {
						...
						//** list filter
						$newsletters_filter_subscribers_list = (!empty($_GET['list'])) ? esc_html($_GET['list']) : false;
						...
						$newsletters_filter_subscribers_expired = (!empty($_GET['expired'])) ? esc_html($_GET['expired']) : $newsletters_filter_subscribers_expired;
						...
						$newsletters_filter_subscribers_status = (!empty($_GET['status'])) ? esc_html($_GET['status']) : $newsletters_filter_subscribers_status;
						...
						//** registered filter
						$newsletters_filter_subscribers_registered = (empty($_GET['registered'])) ? $_COOKIE['newsletters_filter_subscribers_registered'] : esc_html($_GET['registered']);
						...
						//** country filter
						$newsletters_filter_subscribers_country = (empty($_GET['country'])) ? $_COOKIE['newsletters_filter_subscribers_country'] : esc_html($_GET['country']);
		...
		function admin_importexport() {
			...
			if (!empty($_POST)) {
				...
				switch ($method) {
					case 'delete'			:						
						$uploadedfile = map_deep($_FILES['file'], 'sanitize_text_field');
					...
					case 'import'			:
						...
						if (empty($error)) {
							...
							$fileFull = sanitize_text_field($_POST['uploadedfile']);
							...
							if ($fh = fopen($fileFull, "r")) {
								...
								$delimiter = (empty($_POST['delimiter'])) ? $csvdelimiter : sanitize_text_field($_POST['delimiter']);
								$d = 0;
								$i_queries = array();
								...
								while (($row = fgetcsv($fh, "1000", $delimiter)) !== false) {
									...
									if (!empty($row)) {
										...
										$mailinglists = array_map('sanitize_text_field', $_POST['importlists']);
								...
								$uploadedfile = sanitize_text_field($_POST['uploadedfile']);
		...
		function admin_mailqueue() {
			...
			switch ($method) {
				case 'batchbulk'			:
					...
					$emails = array_map('sanitize_text_field', $_POST['emails']);
				...
				case 'deleteemail'			:
					$id = esc_html($_GET['id']);
					$batch = array_map('esc_html', $_GET['batch']);
				...
				case 'deletebatch'			:
					...
					break;
				case 'sendbatch'			:
					$batchkey = array_map('esc_html', $_GET['batch']);
					if (!empty($batchkey)) {
			...
			switch ($method) {
							...
							switch ($_REQUEST['action']) {
								...
								case 'exportall'				:
									...
									if (!empty($_REQUEST['action']) && $_REQUEST['action'] == "export") {
										$email_ids = implode(",", map_deep($_POST['emails'], 'sanitize_text_field'));
				...
				case 'exportsent'		:
					...
					if (!empty($_GET['history_id'])) {
						...
						if ($emails = $Db -> find_all(array('history_id' => esc_sql($_GET['history_id'])), false, array('modified', "DESC"))) {
							/* CSV Headings */
							$data = "";
			...
			switch ($method) {
				...
				default					:				
					...
					if (!empty($_POST)) {
						...
						if (!empty($_FILES)) {
							$_FILES = map_deep($_FILES, 'sanitize_text_field');
							...
							switch ($key) {
								...
								case 'customcsscode'		:
									if (!empty($_POST['customcss']) && $_POST['customcss'] == "Y") {
										...
										$this -> update_option('customcsscode', sanitize_textarea_field($_POST['customcsscode']));
									} else {
